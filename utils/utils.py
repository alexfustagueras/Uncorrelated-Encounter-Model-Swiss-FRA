...
def parquet_file_fingerprint(parquet_path: str) -> Dict[str, Any]:
    """Compute fingerprint from parquet file without loading into DataFrame."""
...
def compute_or_load_norm_stats(
    ...
    # Compute normalization from positions (original method)
...
class TrajectoryDataset(Dataset):
    ...
    def __len__(self):
        # Access shape[0] which is safe for mmap arrays (doesn't load data)
        return int(self.X.shape[0])

    def __getitem__(self, i):
        # Use np.array() to ensure we get a copy of the slice, not a view
        # This prevents loading the entire mmap array into RAM
        x = torch.tensor(np.array(self.X[i]), dtype=torch.float32)
        y = torch.tensor(np.array(self.Y[i]), dtype=torch.float32)
        c = torch.tensor(np.array(self.C[i]), dtype=torch.float32)
        return x, y, c


def _load_cached_dataset(paths: Dict[str, pathlib.Path]):
...
def make_loader(ds: Dataset, bs: int, shuffle: bool) -> DataLoader:
    # Use num_workers=0 to avoid memory duplication with memory-mapped arrays
    # Each worker process would duplicate the memory mapping, causing OOM
    return DataLoader(
        ds,
        batch_size=bs,
        shuffle=shuffle,
        num_workers=0,  # Critical: prevents memory duplication with mmap arrays
...
def build_or_load_dataset(
    ...
    """
    Build or load dataset. If parquet_path is provided and cache exists,
    will skip loading DataFrame to save memory.
    """
    features = ("x", "y", "z", "vx", "vy", "vz", "psi_rate")
    prep = {
        **wparams.to_prep_dict(),
    ...
    samp_dict = samp.to_samp_dict()

    # Try to check cache first WITHOUT loading DataFrame
    if parquet_path is not None and df is None:
        try:
            # Check all .key.json files to find matching cache
            cache_dir = CACHE_DIR
            for key_file in cache_dir.glob("*.key.json"):
                try:
                    key_data = json.loads(key_file.read_text())
                    # Match prep and sampling configs
                    if (key_data.get("prep") == prep and 
                        key_data.get("sampling") == samp_dict):
                        # Found matching cache!
                        dset_key = key_data.get("dataset_key", key_file.stem.replace(".key", ""))
                        stats_key = key_data.get("stats_key", "")
                        if dset_key and stats_key:
                            paths = cache_paths(dset_key, stats_key)
                            if cache_exists(paths):
                                print(f"[cache] hit (no DF load): {dset_key}")
    ...
    # Fall back to normal method (requires DataFrame)
    if df is None:
        raise ValueError("Either df or parquet_path must be provided")
...
def denorm_seq_to_global(
    ...
    seq_norm: (B, T, D)
    ctx_norm: (B, C)
    Returns:  (B, T, D) in global coordinates/units
    """
    ...
    x_local = seq[..., 0].clone()
    y_local = seq[..., 1].clone()
    seq[..., 0] = c * x_local - s * y_local
    seq[..., 1] = s * x_local + c * y_local

    vx_local = seq[..., 3].clone()
    vy_local = seq[..., 4].clone()
    seq[..., 3] = c * vx_local - s * vy_local
    seq[..., 4] = s * vx_local + c * vy_local
...
# ---------------------- Dataset creation from clean data ----------------------
